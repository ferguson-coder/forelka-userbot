...
async def ping_cmd(client, message, args):
    ...
    await message.edit("<blockquote><emoji id=5891211339170326418>⌛️</emoji> <b>Pinging...</b></blockquote>", parse_mode='html')
...
async def inline_ping_handler(event: events.InlineQuery.Event):
    ...
    if not is_owner(kernel, user_id):
        ...
        no_access_result = builder.article(
            ...
            text="<blockquote><emoji id=5778527486270770928>⛔</emoji> <b>Доступ запрещен.</b>\nЭта функция доступна только владельцам юзербота.</blockquote>",
            parse_mode='html'
        )
...
async def bot_ping_handler(event: events.NewMessage.Event):
    ...
    start = time.perf_counter()
    pong_msg = await event.reply("<blockquote><emoji id=5891211339170326418>⌛️</emoji> <b>Pinging...</b></blockquote>", parse_mode='html')
    ms = (time.perf_counter() - start) * 1000
    res = (
        f"<emoji id=5897962422169243693>👻</emoji> <b>Pong</b>\n"
        f"<blockquote><emoji id=5776375003280838798>✅</emoji> <b>Latency:</b> <code>{ms:.2f} ms</code></blockquote>"
