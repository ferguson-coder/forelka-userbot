...
async def update_cmd(client, message, args):
    try:
        await message.edit("<blockquote><emoji id=5891211339170326418>⌛️</emoji> <b>Updating...</b></blockquote>", parse_mode='html')
        res = subprocess.check_output(["git", "pull"]).decode()
        if "Already up to date" in res:
            return await message.edit("<blockquote><emoji id=5775887550262546277>❗️</emoji> <b>Already up to date</b></blockquote>", parse_mode='html')
        
        os.environ["RESTART_INFO"] = f"{time.time()}|{message.chat_id}|{message.id}"
        os.execv(sys.executable, [sys.executable, "main.py"])
    except Exception as e:
        await message.edit(f"<blockquote><emoji id=5778527486270770928>❌</emoji> <b>Error:</b> <code>{e}</code></blockquote>", parse_mode='html')

async def restart_cmd(client, message, args):
    os.environ["RESTART_INFO"] = f"{time.time()}|{message.chat_id}|{message.id}"
    await message.edit("<blockquote><emoji id=5891211339170326418>⌛️</emoji> <b>Restarting...</b></blockquote>", parse_mode='html')
    os.execv(sys.executable, [sys.executable, "main.py"])
